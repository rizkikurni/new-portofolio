    ...
    return (
        <article className="group overflow-hidden rounded-3xl border border-white/10 bg-dark-800 transition-colors hover:border-accent-500/35">
            <div className="grid lg:grid-cols-[0.92fr_1.08fr]">
                <Link href={`/projects/${project.slug}`} className="relative min-h-72 overflow-hidden bg-dark-700 lg:min-h-[420px]" aria-label={`View ${project.title} case study`}>
                    {project.thumbnail ? (
                    ...
                    <span className="absolute left-5 top-5 rounded-full border border-white/15 bg-dark-900/80 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-white backdrop-blur lg:text-xs">
                        {project.is_featured ? 'Featured project' : `Project ${String(index + 1).padStart(2, '0')}`}
                    </span>
                </Link>

                <div className="flex flex-col justify-center p-7 sm:p-9 lg:p-12">
                    <div className="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-gray-500 lg:text-sm">
                    ...
                    <p className="mt-4 max-w-xl text-sm leading-7 text-gray-400 sm:text-base lg:text-lg lg:leading-8">
                    ...
                    {outcome && (
                        <div className="mt-6 flex gap-3 rounded-2xl border border-white/8 bg-white/[0.025] p-4">
                            ...
                            <p className="line-clamp-3 text-sm leading-6 text-gray-300 lg:text-base lg:leading-7">{outcome}</p>
                        </div>
                    )}
                    ...
                    {skills.length > 0 && (
                        <div className="mt-6 flex flex-wrap gap-2">
                            {skills.map((skill) => (
                                <span key={skill.id} className="rounded-lg bg-white/5 px-2.5 py-1.5 text-xs font-medium text-gray-300 lg:text-sm">{skill.name}</span>
                            ))}
                        </div>
                    )}

                    <Link href={`/projects/${project.slug}`} className="mt-8 inline-flex w-fit items-center gap-2 text-sm font-bold text-accent-500 transition-colors hover:text-accent-400 lg:text-base">
                        View case study
