    ...
    return (
        <div className="min-h-screen overflow-x-hidden bg-dark-900 font-sans text-gray-300 antialiased selection:bg-accent-500/25 selection:text-white">
            ...
            <main className="pt-32 sm:pt-36">
                <Container>
                    <Link href="/" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] text-gray-400 transition-colors hover:text-white lg:text-sm">
                        <ArrowLeft className="h-4 w-4" /> Back to portfolio
                    </Link>

                    <header className="grid gap-10 py-14 lg:grid-cols-[1fr_0.42fr] lg:items-end lg:gap-20 lg:py-20">
                        <div>
                            <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[0.14em] text-accent-500 lg:text-sm">
                        ...
                        <div className="space-y-5 border-l border-white/10 pl-6">
                            {(project.start_date || project.end_date) && (
                                <div className="lg:[&>p:last-child]:text-base">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500 lg:text-xs">Timeline</p>
                            ...
                            {project.skills?.length > 0 && (
                                <div className="lg:[&>p:last-child]:text-base lg:[&>p:last-child]:leading-7">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500 lg:text-xs">Core stack</p>
                    ...
                    <div className="grid gap-14 py-20 lg:grid-cols-[1fr_0.38fr] lg:gap-20 lg:py-28">
                        <div className="space-y-16">
                            {project.description && (
                                <section>
                                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-500 lg:text-sm">Overview</p>
                                    <h2 className="mt-4 text-3xl font-extrabold tracking-tight text-white">About the project</h2>
                                    <div className="portfolio-prose mt-6 text-base leading-8 text-gray-300 lg:text-lg lg:leading-9" dangerouslySetInnerHTML={{ __html: project.description }} />
                                </section>
                            )}

                            {hasCaseStudy && caseStudySections.map(({ key, eyebrow, title }) => project[key] && (
                                <section key={key} className="border-t border-white/10 pt-10">
                                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-500 lg:text-sm">{eyebrow}</p>
                                    <h2 className="mt-4 text-3xl font-extrabold tracking-tight text-white">{title}</h2>
                                    <div className="portfolio-prose mt-6 text-base leading-8 text-gray-300 lg:text-lg lg:leading-9" dangerouslySetInnerHTML={{ __html: project[key] }} />
                                </section>
                            ))}
                        </div>
                        ...
                        <aside className="h-fit rounded-3xl border border-white/10 bg-dark-800 p-6 lg:sticky lg:top-28">
                            <div className="flex items-center gap-2 border-b border-white/10 pb-4">
                                ...
                                <h2 className="font-bold text-white lg:text-lg">Project details</h2>
                            </div>
                            {project.skills?.length > 0 && (
                                <div className="mt-5 flex flex-wrap gap-2">
                                    {project.skills.map((skill) => <span key={skill.id} className="rounded-lg bg-white/5 px-2.5 py-1.5 text-xs font-medium text-gray-300 lg:text-sm">{skill.name}</span>)}
                                </div>
                            )}
                            <div className="mt-6 space-y-3">
                                ...
                                {project.github_url && <a href={project.github_url} target="_blank" rel="noopener noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-white/5 lg:text-base"><Github className="h-4 w-4" /> View source code</a>}
                            </div>
                            {project.status && <p className="mt-5 flex items-center gap-2 text-xs font-medium capitalize text-gray-400 lg:text-sm"><CheckCircle2 className="h-4 w-4 text-accent-500" /> {project.status}</p>}
                        </aside>
                    </div>

                    {project.media?.length > 0 && (
                        <section className="border-t border-white/10 py-20 md:py-28">
                            <p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-500 lg:text-sm">Gallery</p>
                            ...
                            <div className="mt-10 grid gap-6 md:grid-cols-2">
                                {project.media.map((media, index) => (
                                    <figure key={media.id} className={`${index === 0 ? 'md:col-span-2' : ''} overflow-hidden rounded-3xl border border-white/10 bg-dark-800`}>
                                        ...
                                        {media.caption && <figcaption className="border-t border-white/10 px-5 py-4 text-sm text-gray-400 lg:text-base">{media.caption}</figcaption>}
                                    </figure>
                                ))}
                            </div>
                        </section>
